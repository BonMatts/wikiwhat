...
describe Wikiwhat::Parse::Results do
  let(:result){Wikiwhat::Parse::Results.new()}
...
describe Wikiwhat::Parse::Text do
  let(:kel){Wikiwhat::Parse::Text.new(ext_output)}
  let(:kel_rev){Wikiwhat::Parse::Text.new(rev_output, "revisions")}
  ...
  describe '#sidebar_image' do
    it "returns the URL of the sidebar image, if any" do
      Wikiwhat::Api::Call.stub(:call_api).and_return(img_url_output)
...
describe Wikiwhat::Parse::Media do
  let(:albert){Wikiwhat::Parse::Media.new(img_output, "pages")}

  describe '#list_images' do
    it "pulls out file names and queries the api for their urls, returns an array of urls" do
      Wikiwhat::Api::Call.stub(:call_api).and_return(media_list_1, media_list_2, media_list_3, media_list_4, media_list_5, media_list_6, media_list_7, media_list_8, media_list_9, media_list_10)
